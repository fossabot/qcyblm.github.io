		...
		if (yamlStr) { // 正确解析出yaml字符串后
			...
			if (dataList && dataList.length) { // 有列表数据
				...
				if (type === CARD_LINK) { // 普通卡片列表1
					listDOM = getCardLinkDOM(dataList, row)
...
// 将数据解析成DOM结构 - 普通卡片列表1
function getCardLinkDOM(dataList, row) {
	let listDOM = ''
	dataList.forEach(item => {
		...
		listDOM += [
			`<a href="${item.link}" title="${title}" class="card-item ${row ? 'row-' + row : ''}" target="_blank" rel="noopener noreferrer" style="${bgColor}">`,
			`<p class="name" style="${nameColor}">${item.icon ? '<img src="' + withBase(item.icon) + '" class="no-zoom" />' : ''}  ${item.name} <OutboundLink /></p>`,
			`</a>\n`].join("")
	})
	return listDOM
}
